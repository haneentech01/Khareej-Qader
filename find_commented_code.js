...
const dirsToSearch = ['app', 'components', 'hooks', 'lib', 'providers', 'types', 'utils', 'services', 'store'];

const isCodeHeuristic = (text) => {
  if (/eslint-disable/i.test(text) || /@ts-/i.test(text) || /TODO/i.test(text) || /FIXME/i.test(text) || /NOTE/i.test(text) || /HACK/i.test(text) || /WARNING/i.test(text)) {
    return false;
  }
...
const findCommentedCode = (filePath) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (jsxCommentMatch) {
       if (isCodeHeuristic(jsxCommentMatch[1]) || /<[a-zA-Z]+/.test(jsxCommentMatch[1])) {
  ...
  if (matches.length > 0) {
    console.log(`\nFile: ${filePath}`);
    matches.forEach(m => console.log(`  Line ${m.line}: ${m.content.trim()}`));
  }
};
