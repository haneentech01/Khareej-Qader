...
export function StudentProfileStats({ student }: StudentProfileStatsProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
      {/* Track Progress */}
      <div className="bg-white border border-slate-100/80 rounded-3xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.015)] hover:shadow-md transition-all duration-300 flex flex-col gap-3">
      ...
      {/* Completed Lessons */}
      <div className="bg-white border border-slate-100/80 rounded-3xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.015)] hover:shadow-md transition-all duration-300 flex items-center justify-between gap-3">
        <div className="size-12 rounded-2xl bg-blue-50 flex items-center justify-center shrink-0">
          <BookOpen className="size-5 text-blue-500" />
        </div>
        <div className="text-right">
          <p className="text-slate-400 text-xs font-bold">{t("stats.completed_lessons")}</p>
          <p className="text-black font-extrabold text-2xl md:text-3xl leading-tight">
            {student.completedLessons}
            <span className="text-base font-bold text-slate-300 mx-1">/</span>
            <span className="text-base font-bold text-slate-400">{student.totalLessons}</span>
          </p>
      ...
      <div className="bg-white border border-slate-100/80 rounded-3xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.015)] hover:shadow-md transition-all duration-300 flex items-center justify-between gap-3">
        ...
        <div className="text-right">
          <p className="text-slate-400 text-xs font-bold">{t("stats.completed_tasks")}</p>
          <p className="text-black font-extrabold text-2xl md:text-3xl leading-tight">
            {student.completedTasks}
            <span className="text-base font-bold text-slate-300 mx-1">/</span>
            <span className="text-base font-bold text-slate-400">{student.totalTasks}</span>
          </p>
