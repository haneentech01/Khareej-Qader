...
interface MentorsTableProps {
  ...
  handleToggleAccount: (mentor: AdminMentor) => Promise<void>;
}
      ...
      gap-4 items-center justify-between">
        ...
        <div className="relative w-full md:w-96">
          ...
          <Input
            ...
            placeholder="البحث باسم المنتور أو البريد..."
      ...
      <div className="overflow-hidden">
        {mentors.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-center px-4">
            ...
            <p className="text-slate-500 font-bold">لا يوجد منتورات مطابقون للبحث</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <Table>
              <TableHeader className="bg-slate-50/70 border-b border-slate-100">
                <TableRow>
                  <TableHead className="py-4 px-6 text-right font-extrabold text-brand-muted">المنتور</TableHead>
                  <TableHead className="py-4 px-6 text-right font-extrabold text-brand-muted">حالة الحساب</TableHead>
                  <TableHead className="py-4 px-6 text-right font-extrabold text-brand-muted">معلومات التواصل</TableHead>
                  <TableHead className="py-4 px-6 text-center font-extrabold text-brand-muted">الإجراءات</TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    >
                      ...
                      <TableCell className="py-4 px-6">
                        <div className="flex items-center gap-3.5 min-w-50">
                          <div className="size-11 rounded-2xl bg-blue-50 text-blue-600 font-black text-base flex items-center justify-center shrink-0">
                            {initial}
                          </div>
                          <div className="space-y-0.5">
                            ...
                            {mentor.created_at && (
                              <span className="flex items-center gap-1 text-xs text-slate-400">
                                ...
                                انضم: {new Date(mentor.created_at).toLocaleDateString(locale)}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <TableCell className="py-4 px-6 text-left whitespace-nowrap">
                        {mentor.account_status ? (
                          <Button
                            onClick={() => handleToggleAccount(mentor)}
                            disabled={isToggling}
                          ...
                          <Button
                            onClick={() => handleToggleAccount(mentor)}
                            disabled={isToggling}
                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl h-10 px-4 gap-2 transition-all shadow-2xs"
                          >
                            {isToggling ? (
                              <Loader2 className="size-4 animate-spin" />
                            ) : (
